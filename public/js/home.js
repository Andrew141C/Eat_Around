...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('closeOrders').addEventListener('click', () => toggleOrders(false));
  document.getElementById('ordersPay').addEventListener('click', payOrdersModal);
...
async function payAndSummarize() {
  try {
    ...
    renderOrdersModal(orders, totalSum);
    toggleOrders(true);
  } catch (err) {
...
function renderOrdersModal(orders, totalSum) {
  ...
  notice.textContent = '';
  if (!orders.length) {
    list.innerHTML = '<div class="orders-group">No orders found.</div>';
    totalEl.textContent = 'Grand total: $0.00';
    return;
  }
  orders.forEach((o) => {
    const row = document.createElement('div');
    row.className = 'orders-group';
    const items = Array.isArray(o.items) ? o.items : [];
    const itemsHtml = items
      .map(
        (it) =>
          `<div>${it.name || 'Item'} x${Number(it.quantity || 0)} ($${Number(it.price || 0).toFixed(
            2
          )})</div>`
      )
      .join('');
    row.innerHTML = `
      <div class="cart-name">${items.length ? 'Items' : 'Order'}</div>
      <div class="orders-items">${itemsHtml || '<div>No items</div>'}</div>
      <div class="meta">Total: $${Number(o.total || 0).toFixed(2)}</div>
    `;
    list.appendChild(row);
  });
  totalEl.textContent = `Grand total: $${Number(totalSum || 0).toFixed(2)}`;
}

function payOrdersModal() {
